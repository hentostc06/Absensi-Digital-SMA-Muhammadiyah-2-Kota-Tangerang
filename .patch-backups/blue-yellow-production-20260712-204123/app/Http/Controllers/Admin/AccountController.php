...
class AccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($data) {
            $role = $data['role'];
            $password = $data['password'] ?: $this->defaultPassword($role);
             ...
             */
            $username = match ($role) {
                'siswa' => $data['username'] ?: $generatedNis,
                default => $data['username'] ?: $this->uniqueUsername($role, $data['name']),
            };
            ...
            if ($role === 'guru') {
                Teacher::create([
                    'user_id' => $user->id,
                    'niy_nbm' => $data['niy_nbm'] ?: $this->nextTeacherNumber(),
        ...
        DB::transaction(function () use ($account, $data) {
            ...
            if ($account->role === 'guru') {
                $account->teacher()->updateOrCreate(
                    ['user_id' => $account->id],
                    [
                        'niy_nbm' => $data['niy_nbm'] ?: ($account->teacher->niy_nbm ?? $this->nextTeacherNumber()),
                        'phone' => $data['phone'] ?? null,
                    ]
                );
            }
